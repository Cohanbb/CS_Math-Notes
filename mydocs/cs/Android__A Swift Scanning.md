...
四大组件分别是：活动（Activity）用于表现功能，一般一个页面是一个 Activity；服务（Service），后
台运行服务，不提供界面呈现；广播接收者（BroadcasC Receiver），对发送出来的广播进行过滤接受并
响应；内容提供者（Content Provider）不同应用程序之间进行数据交换的标准 API，以 URI 的形式对外
提供数据，允许其他应用操作本应用程序。
...
保存的数据以文件的方式上传到服务器、发送邮件等。具体内容将在第 9 章中详细介绍。
