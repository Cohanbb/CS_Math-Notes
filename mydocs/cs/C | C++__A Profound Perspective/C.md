...
1. C95 补充了宽字符类型以及头文件 wchar_t.h，通过 `typedef unsigned int wchar_t` 实现，并不是一个独立的类型。

  > C++ 中 `wchar_t` 是一个基本数据类型。
...
在 C 语言中，使用 `#define` 定义的称作常量，不会占用内存；使用 `const` 定义的称作**常变量**，虽然值不可改变，但是会占用内存。C 语言中使用 `const` 可以不进行初始化，但是之后也无法改变常变量的值。
...
1. 指针加/减数字：`p + i` 表示指针所指向位置后移 `i` 个单位的位置，`p - i` 表示指针所指位置前移 `i` 个单位的位置。 指针减指针，得到两指针之间的元素个数。
2. 指针的关系运算，`p > q` 代表 `p` 指向的内存地址比 `q` 指向的内存地址大。`p == q` 代表 `p` 和 `q` 指向同一内存地址。
...
int strncmp(char const *s1, char const *s2, size_t count); // 比较两个字符串的前几位
```
...
> 注意：以地址作为参数时，如果不希望改变地址上的值，最好使用 `const` 限定形参为常量指针。

C99 新增可变参数的函数，在最后一个参数的位置使用 `...` 表示可变参数，`var_list` `var_args` `var_`。
...
```c
int *p = (int *)malloc(sizeof (int) * 10); // 动态创建 int 型数组 p，长度为 10
```
