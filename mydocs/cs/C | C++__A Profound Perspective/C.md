...
数组名代表数组首元素的地址，这与对数组名取地址在数值上相同，但代表不同的含义。
...
/*
 * 多维数组的初始化（以二维为例）
 * store_type data_type identifier[length1][length2] = {{value_list1}, {value_list2}};
 */
int a[2][3] = {{1, 2, 3}, {4, 5, 6}};
```

多维数组的本质是以数组为元素的数组，上述的 `a[2][3]` 中，数组 `a` 有两个元素 `a[0]` 和 `a[1]`，这两个元素的数据类型是 `int [3]` 也就是长度为 3 的 int 型一维数组。 

多维数组的数组名代表数组首个元素的地址，即 `a` 代表 `a[0]` 的地址，`a[0]` 是 `int [3]` 类型，所以数组名 `a` 可看作的是 `int (*)[3]` 类型的指针。

`a[0][3]` 是一个长度为 3  的 int 型一维数组，`a[0]` 是这个数组的数组名，代表一维数组首元素的地址。

Demo：

```c
int b[2][3] = {{1, 2, 3}, {4, 5, 6}};
int (*p)[3] = b; // b 代表多维数组首元素的地址，int (*)[3] 型
int (*q)[2][3] = &b; // &b 代表整个多维数组的地址，int (*)[2][3] 型
int *r = b[0]; // b[0] 代表一维数组首元素的地址，int 型
```
